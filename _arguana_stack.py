#!/usr/bin/env python3
"""ARGUANA — the only corpus where we have never run anything but BM25.
...
d = os.path.join(ROOT, "arguana"); corpus = {}; queries = {}
...
print("=" * 96); print(f"ARGUANA FULL STACK | {N:,} docs | {len(ids)} queries | UNCAPPED queries"); print("=" * 96, flush=True)
...
cache = os.path.join(HERE, "_sr_bge-large_arguana.npz")
...
json.dump({"corpus": "arguana", "n_queries": len(ids), "pq_bytes_per_doc": round(bpd, 1), "arms": res},
          open(os.path.join(HERE, "_arguana_stack.json"), "w"), indent=2)
