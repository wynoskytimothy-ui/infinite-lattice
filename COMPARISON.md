...
| **fiqa** (57,638) | **EdgeRAG** (word+bridges) | **176 ms** | **401** | 18.7 ms | **0.2448** | no |
...
- **Query speed:** EdgeRAG fastest — lexical 0.04–0.26 ms; with bridges 0.4–18.7 ms (the bridge rerank is the
  slow part on big corpora, still faster than BM25's 5.7–92 ms). Dense 2.8–3.0 ms (+ encode the query).
...
4. **Bridge query cost** grows with corpus (18.7 ms on fiqa) — the rerank is O(segment) per doc-term, an
   optimization target; still < BM25 there.
